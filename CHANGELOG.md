...
The Cursor CLI provider retains the existing protocol and preservation contract.
...
declares.** The rule a model reads while working on Cursor CLI said the product
...
**A walk written when a surface held one kind.** Cursor CLI's table gained a
...
**Cursor CLI's table gained a kind.** Where a product loads a plugin from a
...
**Cursor CLI declares the `instruction` kind**, and the surface that routes it is
...
Cursor CLI's provider publishes a projection profile per *scope* -- a second
...
Starts Cursor CLI, and takes over a target the estate before this one
still claims.
...
Installs Cursor CLI itself, not only its configuration.
...
Cursor CLI harness configuration in a caller-named target directory.
