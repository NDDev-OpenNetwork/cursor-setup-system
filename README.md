...
cursor-setup-system select minimal      --target ~/.tool-config
...
cursor-setup-system restore --backup slot-000000000002 --target ~/.tool-config
...
Point `CURSOR_SETUP_SYSTEM_SETUP_CATALOG` at a directory to use setups of your own.

## What it manages

| | |
| --- | --- |
| Product | Cursor CLI (Anysphere) |
| Documented configuration home | `~/.cursor` |
...
```bash
CURSOR_INSTALL_DIR=/opt/cursor-setup-system sh install.sh
```

The same variable on both scripts, and it is `CURSOR_INSTALL_DIR`
...
ai-stp provider conformance --harness cursor \
  --executable ~/.local/bin/cursor-setup-system \
...
docker run --rm -v "$HOME/.config:/config" \
  ghcr.io/nddev-opennetwork/cursor-setup-system:0.0.7 \
